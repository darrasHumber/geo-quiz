...
class FlagQuiz {
  constructor() {
    ...
    // Make sure we correctly access these elements after getting settingsModal
    if (this.settingsModal) {
      this.closeSettingsBtn = this.settingsModal.querySelector(".close-btn");
    ...
    this.questionCountDisplay = document.getElementById(
  ...
  initEventListeners() {
    ...
    if (this.questionCountSlider && this.questionCountDisplay) {
      this.questionCountSlider.addEventListener("input", () => {
        this.questionCountDisplay.textContent = `${this.questionCountSlider.value} Questions`;
      });
    }
  }
  ...
  openSettingsModal() {
    ...
    // Make sure the region option exists before trying to check it
    const regionInput = document.querySelector(
      `input[name="region"][value="${this.settings.region}"]`
    );
    if (regionInput) {
    ...
    if (this.questionCountSlider) {
      this.questionCountSlider.value = this.settings.questionCount;
      this.questionCountDisplay.textContent = `${this.settings.questionCount} Questions`;
    }
  ...
  applySettings() {
    ...
    this.settings.region = regionRadio ? regionRadio.value : "all";

    // Get question count
    if (this.questionCountSlider) {
      this.settings.questionCount = parseInt(this.questionCountSlider.value);
    }
    ...
    this.settings.difficulty = difficultyRadio ? difficultyRadio.value : "easy";
